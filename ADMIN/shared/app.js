(function () {
  ...
  function initProducts() {
    ...
    function renderVariants() {
      els.variantRows.innerHTML = variants.map(function (variant, index) {
        return '<tr><td>' + variant.name + '</td><td>' + variant.count + '</td><td><button class="edit-icon" type="button" aria-label="Sửa biến thể"></button><button class="delete-icon" type="button" data-action="delete-variant" data-index="' + index + '" aria-label="Xóa biến thể"></button></td></tr>';
      }).join('');
    }
  ...
  function initContent() {
    ...
    const defaultContentImage = '../shared/Image/z8039620815491_4df5323f01eef523920b1590915a6e67.jpg';
    ...
    function openForm(type, id) {
      ...
      const previewModeMap = { banner: 'banner', collection: 'collections', inspiration: 'inspirations' };
      const titleMap = {
        banner: isEdit ? 'Chỉnh sửa Banner' : 'Thêm Banner mới',
        inspiration: isEdit ? 'Chỉnh sửa Nguồn cảm hứng' : 'Thêm Nguồn cảm hứng',
        collection: isEdit ? 'Chỉnh sửa Collection' : 'Thêm Collection mới',
        idea: isEdit ? 'Chỉnh sửa ý tưởng' : 'Thêm ý tưởng',
        blog: isEdit ? 'Chỉnh sửa Blog' : 'Thêm Blog'
      };
      function formPreview() {
        if (type === 'idea' || type === 'blog') {
          return '<div class="site-preview-frame room-site"><div class="site-preview-top">Chỉ duy nhất 1 lần, tặng voucher 100.000 VNĐ cho khách hàng mới của CaraVeso</div><div class="site-preview-nav"><strong>CaraVeso</strong></div><div class="site-hero" style="background-image:url(' + itemImage + ')"></div><div class="site-tile-row"><span style="background-image:url(' + defaultContentImage + ')"></span><span style="background-image:url(' + defaultContentImage + ')"></span><span style="background-image:url(' + defaultContentImage + ')"></span></div></div>';
        }
        return renderWebsitePreview(previewModeMap[type] || activeHomeTab);
      }
      ...
      if (type === 'collection') {
        html += '<div class="collection-fields"><label>Tên Collection:<input type="text" name="name" value="' + (item ? item.name : '') + '" placeholder="Nhập tên Collection"></label><label>Slogan:<textarea maxlength="40" placeholder="Nhập slogan cho Collection">Đa dạng lựa chọn, vẻ đẹp sang trọng</textarea><small>0/40</small></label></div>';
        ...
        html += '<label class="wide-field">Mô tả:<textarea maxlength="500" placeholder="Nhập mô tả cho Collection">Bộ sưu tập nội thất phòng khách hiện đại và sang trọng.</textarea><small>0/500</small></label>';
        html += '<div class="collection-bottom"><section><h3>Sản phẩm trong Collection (2/3)<b>*</b></h3><div class="collection-images">' + imagePlaceholder('collection-shot', defaultContentImage, '') + imagePlaceholder('collection-shot', defaultContentImage, '') + '<button type="button">▧<small>Thêm hình ảnh</small></button></div></section>' + productAttachedTable('product-list', 'Danh sách sản phẩm', ['<div class="attached-row"><span>1</span><strong>Ghế sofa Milano Shape U</strong><span>#HHT080906</span>' + iconButtons('attached', 'P01') + '</div>', '<div class="attached-row"><span>2</span><strong>Ghế sofa Milano Shape U</strong><span>#HHT080906</span>' + iconButtons('attached', 'P02') + '</div>']) + '</div>' + formActions(isEdit ? 'Cập nhật' : 'Thêm');
      }
      if (type === 'idea') {
      ...
      els.formBody.innerHTML = '<div class="content-form-workspace">' + html + '</form><aside class="website-preview content-form-preview"><h2>Xem trước trên Website</h2>' + formPreview() + '</aside></div>';
    }
