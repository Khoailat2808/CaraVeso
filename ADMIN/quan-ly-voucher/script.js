...
async function init(){
  [vouchers, promotions] = await Promise.all([
    getJson('../../data/admin/vouchers.json', fallbackVouchers),
    getJson('../../data/admin/promotions.json', fallbackPromotions)
  ]);
  ...
  $$('#sideNav .side-item').forEach(item => item.addEventListener('click', () => {
    ...
    const target = pageLinks[item.dataset.page];
    if(target && target !== 'voucher.html') window.location.href = target;
  }));
