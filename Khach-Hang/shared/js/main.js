...
/* Giỏ hàng khởi tạo bằng mock data để demo trang ../gio-hang/.
...
function buildSearchBox(extraClass) {
  ...
  function doSearch(q) {
    ...
    addRecentSearch(q);
    window.location.href = '../san-pham/?q=' + encodeURIComponent(q);
  }
...
function openImageSearchModal() {
  ...
  function runImageSearch() {
    ...
    setTimeout(function () { window.location.href = '../san-pham/' + q; }, 700);
  }
...
/** Link cho 1 mục danh mục:
    - Sản phẩm → lưới ../san-pham/ đã lọc theo danh mục
    - Các phòng → trang phòng ../phong/?room=<slug> */
function catalogHref(key, g, label) {
  if (key === 'rooms') return '../phong/?room=' + encodeURIComponent(g.slug);
  return '../san-pham/?category=' + encodeURIComponent(g.cat) +
...
function buildRoomsPanel(cat) {
  ...
  (cat.groups || []).forEach(function (g) {
    ...
    card.href = '../phong/?room=' + encodeURIComponent(g.slug);
...
function renderHeader() {
  ...
  if (!mount) return;
  const signedInName = sessionStorage.getItem('caraveso_customer_name');
  ...
  topbar.innerHTML =
    '<div class="topbar__inner">' +
      ...
      '<a class="topbar__auth" href="' + (signedInName ? '../tai-khoan/' : '../dang-nhap/') + '">' +
        (signedInName || 'Đăng nhập/ Đăng ký') +
      '</a>' +
    '</div>';
  ...
  NAV_ITEMS.forEach(function (item) {
    ...
    if (hasMega) {
      // Mega menu opens on hover/focus; the main nav link still navigates.
  ...
  openModal(
    '<div class="qa">' +
      '<div class="qa__left">' +
        ...
        '<a class="qa__detail-link" href="../chi-tiet-san-pham/?id=' + product.id + '">Xem chi tiết sản phẩm</a>' +
      '</div>' +
      '<div class="qa__right">' +
...
function renderCartPopup(popup) {
  ...
  if (!cartState.length) {
    ...
    cartState.slice().reverse().forEach(function (item) {
      ...
      row.innerHTML =
        '<a class="cart-popup__img" href="../chi-tiet-san-pham/?id=' + p.id + '"><img src="' + p.img + '" alt="' + p.name + '"></a>' +
        '<div class="cart-popup__info">' +
          '<a class="cart-popup__name" href="../chi-tiet-san-pham/?id=' + p.id + '">' + p.name + '</a>' +
...
function buildProductCard(product) {
  ...
  imgWrap.href = '../chi-tiet-san-pham/?id=' + product.id;
  const img = el('img');
  img.src = product.img;
  ...
  nameLink.href = '../chi-tiet-san-pham/?id=' + product.id;
  ...
  function renderList() {
    ...
    if (!cartState.length) {
      ...
      empty.innerHTML = '<p style="font-size:20px;margin-bottom:14px">Giỏ hàng của bạn đang trống.</p>' +
        '<a class="btn-primary" href="../san-pham/">Tiếp tục mua sắm</a>';
    ...
    cartState.forEach(function (item) {
      ...
      imgWrap.href = '../chi-tiet-san-pham/?id=' + p.id;
...
/* ============ 11. LUỒNG THANH TOÁN ============ */
/* Luồng theo Figma:
   Giỏ hàng → ../thanh-toan/ (địa chỉ + phương thức)
     • COD                  → ../dat-hang-thanh-cong/
     • Momo/chuyển khoản    → ../phuong-thuc-thanh-toan/ → ../cong-thanh-toan/?method=momo|bank
                              → (quét QR / bấm "Tại đây") → ../dat-hang-thanh-cong/
...
function initCheckoutPage() {
  ...
  form.addEventListener('submit', function (e) {
    ...
    if (method === 'cod') {
      // COD: hoàn tất luôn
      window.location.href = '../dat-hang-thanh-cong/?' + orderQuery;
    } else {
      // Momo / chuyển khoản: sang trang chọn cổng thanh toán
      window.location.href = '../phuong-thuc-thanh-toan/?' + orderQuery;
    }
  });
}
...
function initPaymentMethodPage() {
  ...
  btn.addEventListener('click', function () {
    ...
    window.location.href = '../cong-thanh-toan/?method=' + method +
...
function initGatewayPage() {
  ...
  const successUrl = '../dat-hang-thanh-cong/?order=' + params.order + '&total=' + params.total;
  ...
  document.getElementById('gw-brand-icon').innerHTML = isMomo
    ? '<img class="momo-logo momo-logo--sm" src="../shared/img/logo_momo.png" alt="MoMo">'
    : ICONS.bank;
  ...
  if (isMomo) {
    ...
    qrWrap.innerHTML =
      ...
      '<img src="../shared/img/qr_momo.png" alt="Mã QR thanh toán MoMo"></button>';
  } else {
    ...
    qrWrap.innerHTML =
      ...
      '<img src="../shared/img/qr_bank.png" alt="Mã QR VietQR — napas 247"></button>';
  }
  ...
  backLink.addEventListener('click', function (e) {
    ...
    openCancelWarning(function () {
      window.location.href = '../phuong-thuc-thanh-toan/?order=' + params.order + '&total=' + params.total;
    });
  });
    ...
    if (remain <= 0) {
      ...
      openModal(
        ...
        '<div class="modal__actions">' +
          '<a class="btn-primary" href="../gio-hang/">Về giỏ hàng</a>' +
...
function initLoginPage() {
  ...
  form.addEventListener('submit', function (e) {
    ...
    if (ok) {
      sessionStorage.setItem('caraveso_customer_name', 'Hoang Huy Tien');
      showToast('Đăng nhập thành công!');
      setTimeout(function () { window.location.href = '../trang-chu/'; }, 1200);
    }
  });
}
...
function initRegisterPage() {
  ...
  form.addEventListener('submit', function (e) {
    ...
    if (okFields && okAgree) {
      ...
      setTimeout(function () { window.location.href = '../trang-chu/'; }, 900);
    }
  });
}
...
/* ============ TRANG CÁC PHÒNG (../phong/) & PHONG CÁCH (../phong-cach/) ============ */
...
function buildHotspots(cell, hotspots) {
  (hotspots || []).forEach(function (h) {
    ...
    tip.href = '../chi-tiet-san-pham/?id=' + p.id;
    tip.innerHTML =
...
function initRoomPage() {
  ...
  room.categories.forEach(function (cat, i) {
    ...
    a.innerHTML = '<img src="../shared/img/products/' + catImg + '" alt="' + cat + '" loading="lazy">';
    catRow.appendChild(a);
  });

  /* --- Ý tưởng: 4 card phong cách, KHÁM PHÁ -> ../phong-cach/ --- */
  document.getElementById('room-idea-title').textContent = room.ideaTitle;
  const ideaRow = document.getElementById('idea-row');
  Object.keys(STYLES).forEach(function (key) {
    const s = STYLES[key];
    const href = '../phong-cach/?style=' + key + '&room=' + roomId;
    const ideaImg = (room.ideaImages && room.ideaImages[key]) || (STYLES[key] && STYLES[key].heroImg) || '../shared/img/rooms/phòng ngủ.jpg';
...
function initChatboxAiPage() {
  ...
  function productCard(product) {
    const image = product.img || '../shared/img/products/ban.webp';
    const article = el('article', 'chat-product-card');
    article.innerHTML =
      '<a class="chat-product-card__img" href="../chi-tiet-san-pham-chatbox/">' +
        '<img src="' + image + '" alt="' + product.name + '" loading="lazy">' +
      '</a>' +
      '<h3><a href="../chi-tiet-san-pham-chatbox/">' + product.name + '</a></h3>' +
      '<p>' + product.desc + '</p>' +
...
function initChatboxProductDetailPage() {
  ...
  CHAT_PRODUCTS.slice(1, 6).forEach(function (product) {
    ...
    card.innerHTML =
      '<a class="chat-product-card__img" href="../chi-tiet-san-pham-chatbox/"><img src="' + product.img + '" alt="' + product.name + '" loading="lazy"></a>' +
      '<h3><a href="../chi-tiet-san-pham-chatbox/">' + product.name + '</a></h3>' +
      '<p>' + product.desc + '</p>' +
      '<strong>' + fmtVND(product.price) + '</strong>' +
  ...
  if (page === 'account-orders') {
    ...
    ACCOUNT_ORDERS.forEach(function (order) {
      ...
      row.innerHTML = '<td>' + order.id + '</td><td>' + order.date + '</td><td>' + fmtVND(order.total) +
        '</td><td>' + order.status + '</td><td><a href="../chi-tiet-don-hang/?order=' + order.id + '">Chi tiết</a></td>';
